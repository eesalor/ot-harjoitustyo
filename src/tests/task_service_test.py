...
class TestTaskService(unittest.TestCase):
    ...
    def test_create_task_without_category(self):
        category_none = None
        self.task_service.create_task(self.title, self.date, category_none)

        self.assertEqual(len(self.task_service.get_tasks(False)), 1)

    def test_create_task_with_empty_category(self):
        self.task_service.delete_category("Studies")
        category_entry = ""
        self.task_service.create_task(self.title, self.date, category_entry)

        self.assertEqual(self.task_service.get_categories(), [])

    def test_create_task_with_category(self):
        category = self.category
        result = self.task_service.create_task(self.title, self.date, category)
    ...
    def test_create_task_with_invalid_task_date_entry(self):
        with self.assertRaises(InvalidTaskError):
            self.task_service.create_task(title="Code", date="This is not date in form dd.mm.yyyy",
                                     category=None)
    ...
    def test_get_uncompleted_tasks(self):
        ...
        result = self.task_service.get_tasks(completion_status)
    ...
    def test_set_completed(self):
        result = self.task_service.set_completed(1)
    ...
    def test_set_uncompleted(self):
        ...
        result = self.task_service.set_uncompleted(1)
    ...
    def test_delete_category(self):
        self.task_service.delete_category(self.category)
    ...
    def test_delete_category_deletes_the_category_from_tasks(self):
        self.task_service.create_task(self.title, self.date, self.category)
